...
class player extends MovingObject {
  ...
  draw(ctx) {
    ...
    ctx.save();
    ctx.translate(this.pos[0] + anglePack[1] * 40, this.pos[1] - anglePack[2] * 40);
    ctx.rotate(anglePack[0] * Math.PI / 180);
    const spritePos = this.vel[0] === this.vel[1] && this.vel[0] === 0 ? spriteStill : spriteWalk;
    this.spriteIndex = (this.spriteIndex + 0.5) % spritePos.length;
    ...
    ctx.drawImage(image, spritePos[idx][0], spritePos[idx][1], 120, 120, 0, 0, 80, 80);
    ctx.restore();
    debugger;
  }
}
