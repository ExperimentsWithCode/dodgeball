...
var Static_Object = function () {
  function Static_Object(options) {
    ...
    this.size = [50, 50]; //options.size || 300;
    this.game = options.game;
    this.isWrappable = false;
  }
  ...
  _createClass(Static_Object, [{
    ...
    value: function draw(ctx) {
      ...
      ctx.beginPath();
      ctx.arc(this.pos[0], this.pos[1], 30, 0, 2 * Math.PI, true);
      ctx.fill();
      var image = new Image();
      image.src = "./img/sportsDrink.png";
      ctx.drawImage(image, 0, 0, 500, 500, this.pos[0] - this.size[0] / 2, this.pos[1] - this.size[1] / 2, 50, 50);
